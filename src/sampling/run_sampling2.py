...
def forward_model(box_size     = [200., 200., 2000.], # In Mpc/h
    ...
    for i,k in enumerate(convergence_maps):
        ...
        observed_maps.append( numpyro.sample('kappa_%d'%i, dist.Normal(k, sigma_e/jnp.sqrt(galaxy_density*(field_size*60/field_npix)**2))))
    
    #np.save('/pscratch/sd/y/yomori/lensing_fwd_mdl_nbody_0/kappa.npy',convergence_maps[0] )
    
    filename = "/pscratch/sd/y/yomori/kappa{}.npy"
    counter = 0
    ...
    #import pdb;pdb.set_trace()
    jnp.save(filename,convergence_maps[0] )
...
for p in Path("/pscratch/sd/y/yomori/").glob("kappa*.npy"):
    p.unlink()
mcmc.run(jax.random.PRNGKey(0))
print("-----------------DONE SAMPLING---------------------")

res = mcmc.get_samples()
with open('/pscratch/sd/y/yomori/lensing_fwd_mdl_nbody_0.pickle', 'wb') as handle:
