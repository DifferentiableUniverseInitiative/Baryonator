...
if resume_state<0:
    ...
    with open('/pscratch/sd/y/yomori/francois_lensing_fwd_mdl_nbody_matched5_0.pickle', 'wb') as handle:
        pickle.dump(res, handle, protocol=pickle.HIGHEST_PROTOCOL)
    del res

    final_state = mcmc.last_state
    with open('/pscratch/sd/y/yomori/francois_mcmc_state_matched5_0.pkl', 'wb') as f:
    ...
    for i in range(1,500):
        ...
        mcmc.run(mcmc.post_warmup_state.rng_key)
        res = mcmc.get_samples()
        with open('/pscratch/sd/y/yomori/francois_lensing_fwd_mdl_nbody_matched5_%d.pickle'%(i), 'wb') as handle:
            pickle.dump(res, handle, protocol=pickle.HIGHEST_PROTOCOL)
        del res

        final_state = mcmc.last_state
        with open('/pscratch/sd/y/yomori/francois_mcmc_state_matched5_%d.pkl'%i, 'wb') as f:
            pickle.dump(final_state, f)

else:
    # Save
    with open('/pscratch/sd/y/yomori/francois_mcmc_state_matched5_%d.pkl'%resume_state, 'rb') as f:
